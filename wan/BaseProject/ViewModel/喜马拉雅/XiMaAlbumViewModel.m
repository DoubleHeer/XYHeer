...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask = [XiMaNetManager getMusicCategoryAlbumId:_albumId PageId:_pageId completeHandle:^(MusicCategoryModel *model, NSError *error) {
        if (_pageId == 1) {
...
-(MusicCategoryTracksListModel *)modelForRow:(NSInteger)row{
...
-(NSURL *)playURLForRow:(NSInteger)row{
    //NSLog(@"%@",[self modelForRow:row].playUrl64);
